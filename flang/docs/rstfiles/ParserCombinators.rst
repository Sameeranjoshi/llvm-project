...
.. raw:: html
   ...
   <!--===- documentation/ParserCombinators.rst 
...
#. provides a function (\ ``const`` member or ``static``\ ) that accepts a reference to a
...
* ``construct<T>(p1, p2, ...)`` matches zero or more parsers in succession,
  ...
  value but only success or failure (\ *e.g.,* ``"IF"_tok``\ ), the default
