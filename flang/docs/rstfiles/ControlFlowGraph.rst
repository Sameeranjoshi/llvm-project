...
.. raw:: html
   ...
   <!--===- documentation/ControlFlowGraph.rst 
...
A *control flow graph* is a collection of simple (\ *i.e.,* "non-extended")
basic *blocks* that comprise straight-line sequences of *actions* with a
single entry point and a single exit point, and a collection of
directed flow *edges* (or *arcs*\ ) denoting all possible transitions of
...
Each basic block comprises a linear sequence of *actions*.
